...
export const SolvedIssues = ({ useremail }) => {
  ...
  useEffect(() => {
    ...
    const fetchIssues = async () => {
      try {
        ...
        const solvedIssues = data.filter(issue => issue.status === "Solved");
  ...
  return (
    <div className="p-0">
      {loading ? (
            ...
            >
              ...
              <div className="mt-3 md:mt-0 md:self-center">
                <div className="shadow-md bg-white rounded-lg p-3">
                  <span className="px-3 py-1 rounded-full text-sm font-semibold bg-green-200 text-green-700">
                    {issue.status}
                  </span>
                </div>
              </div>
        ...
        <p className="text-center text-gray-500">No solved issues available.</p>
      )}
    </div>
  );
};
